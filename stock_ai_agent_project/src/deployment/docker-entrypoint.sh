...
# Clean any existing gcloud configuration to avoid old project settings
rm -rf /root/.config/gcloud

# Authenticate gcloud using service account
gcloud auth activate-service-account --key-file $GOOGLE_APPLICATION_CREDENTIALS

# Force set the correct project (no prompts)
...
if ! gsutil ls -b $PULUMI_BUCKET >/dev/null 2>&1; then
    echo "Bucket does not exist. Creating..."
    gsutil mb -p $GCP_PROJECT -l $PULUMI_BUCKET_REGION $PULUMI_BUCKET
else
    echo "Bucket already exists. Skipping creation."
fi
