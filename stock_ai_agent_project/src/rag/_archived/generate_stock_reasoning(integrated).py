...
def generate_reasoning_for_dataframe(
    ...
    df_enhanced = process_csv_with_rag(df, chain, sample_size=sample_size, max_workers=max_workers)
...
def add_reasoning_to_combined_file(
    ...
    df_enhanced = generate_reasoning_for_dataframe(df, chain, sample_size=sample_size, max_workers=max_workers)

    # Determine output path
    if output_path is None:
        input_path_obj = Path(combined_file_path)
        output_path = str(input_path_obj.parent / f"{input_path_obj.stem}_with_reasoning{input_path_obj.suffix}")
...
def run_pipeline_with_reasoning(
    ...
    if rag_enabled:
        ...
        rag_sample_size = rag_sample_size if rag_sample_size is not None else rag_config.get("sample_size")
        rag_max_workers = rag_max_workers if rag_config.get("max_workers") else rag_max_workers
        print(f"RAG Reasoning: Enabled (max_workers={rag_max_workers}, sample_size={rag_sample_size})")
    else:
    ...
    try:
        ...
        if step == "all":
            results = run_full_pipeline(force_refresh=force_refresh, skip_training=skip_training)
        elif step == "backtest":
        ...
        if rag_enabled and results and results.get("agent_files"):
            ...
            if combined_file and os.path.exists(combined_file):
                ...
                try:
                    ...
                    output_path = str(
                        combined_path_obj.parent / f"{combined_path_obj.stem}_with_reasoning{combined_path_obj.suffix}"
                    )

                    # Get GCS paths from config
                    gcs_bucket = config.get("gcs", {}).get("bucket_name", GCS_BUCKET_NAME)
                    gcs_output_folder = config.get("gcs", {}).get("output_folder", "model_output")
                    gcs_output_path = (
                        f"{gcs_output_folder}/combined_quantamental_hybrid_with_factors_and_backtest_with_reasoning.csv"
                    )
                    ...
                    if "agent_files" in results:
                        results["agent_files"]["combined_with_reasoning"] = enhanced_file
                    ...
                    print("Pipeline completed successfully, but reasoning was not added.")
                    import traceback
...
def main():
    ...
    parser.add_argument("--force-refresh", action="store_true", help="Force refresh data from API (pipeline mode)")
    parser.add_argument("--skip-training", action="store_true", help="Skip model training (pipeline mode)")
    parser.add_argument("--disable-rag", action="store_true", help="Disable RAG reasoning generation (pipeline mode)")
